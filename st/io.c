...
#ifdef WIN32
    //在windows下没有象linux一样的pipe管道函数，所以用winsock代替
    /* Winsock Data */
                      ...
                      st_utime_t timeout)
{
  ...
  _st_netfd_t *newfd;
#ifdef WIN32
  int isoket=fds[fd->osfd];
  //qDebug() <<"accept:" << isoket;
  while(osfd<0)
  {
      osfd = accept(fds[fd->osfd], addr, (socklen_t *)addrlen);
      ...
      //printf("accept error: socket=%d, osfd=%d ",fds[fd->osfd],osfd);
  ...
  while ((osfd = accept(fd->osfd, addr, (socklen_t *)addrlen)) < 0)
  {
    ...
    {
        //printf("accept error: _IO_NOT_READY_ERROR ");
      return NULL;
    }
    /* Wait until the socket becomes readable */
    if (st_netfd_poll(fd, POLLIN, timeout) < 0)
    {
        //printf("accept error: st_netfd_poll<0 ");
      return NULL;
    }
  }
               ...
               st_utime_t timeout)
{
  int n, err = 0;
#ifdef WIN32
  while(connect(fds[fd->osfd], addr, addrlen) < 0)
   {
    errno=_st_GetError(0);
#else
...
ssize_t st_read(_st_netfd_t *fd, void *buf, size_t nbyte, st_utime_t timeout)
{
  ...
  while ((n = read(fd->osfd, buf, nbyte)) < 0) {
    ...
    //POLLIN POLLIN|POLLPRI类似于select的读事件
		 ...
		 st_utime_t timeout)
{
  ssize_t n;
#ifdef WIN32
  while((n = readv(fds[fd->osfd], iov, iov_size)) < 0)
  {
   errno=_st_GetError(0);
#else
		   ...
		   st_utime_t timeout)
{
  ...
  while (*iov_size > 0) {
    ...
    {
#ifdef WIN32
      n = recv(fds[fd->osfd], (*iov)->iov_base, (*iov)->iov_len,0);
#else
    ...
    {
#ifdef WIN32
	n = readv(fds[fd->osfd], *iov, *iov_size);
#else
...
#ifdef _WIN32
    if ((n = writev(fds[fd->osfd], tmp_iov, iov_cnt)) < 0) {
    	errno=_st_GetError(0);
#else
		    ...
		    st_utime_t timeout)
{
  ...
  while (*iov_size > 0) {
    ...
    {
#ifdef WIN32
      n = send(fds[fd->osfd], (*iov)->iov_base, (*iov)->iov_len,0);
#else
      n = write(fd->osfd, (*iov)->iov_base, (*iov)->iov_len);
    	...
    	{
#ifdef WIN32
		n = writev(fds[fd->osfd], *iov, *iov_size);
#else
                ...
                int *fromlen, st_utime_t timeout)
{
  int n;
#ifdef WIN32
  while ((n = recvfrom(fds[fd->osfd], buf, len, 0, from, (socklen_t *)fromlen))< 0) {
    errno=_st_GetError(0);
#else
...
int st_sendto(_st_netfd_t *fd, const void *msg, int len,const struct sockaddr *to,
